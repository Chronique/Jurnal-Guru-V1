...
import { Save, X, Calendar, Clock, BookOpen, Users, FileText, Clock as ClockIcon, Camera, Image as ImageIcon } from 'lucide-react';
...
const ATTENDANCE_OPTIONS: {
  ...
  },
  {
    ...
    inactive: 'bg-white text-slate-400 border-slate-200 hover:bg-slate-50',
    summaryBg: 'bg-rose-50 border-rose-100',
    summaryText: 'text-rose-700',
    summaryValue: 'text-rose-600',
  },
];
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input type="text" name="subject" required placeholder="Contoh: Matematika" value={formData.subject} onChange={handleChange}
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors text-slate-900" />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <textarea name="topic" required rows={3} placeholder="Jelaskan materi yang diajarkan hari ini..." value={formData.topic} onChange={handleChange}
        ...
        <div className="space-y-4">
          ...
          {/* Tabel siswa — tombol H/S/I/A kotak kecil */}
                            ...
                            {ATTENDANCE_OPTIONS.map(opt => (
                              <button
                                ...
                                className={`w-8 h-8 rounded-lg border text-xs font-bold transition-all ${
                                  current === opt.status
                                    ? `${opt.active} ring-2 ring-offset-1`
                                    : opt.inactive
                                }`}
                              >
                                {opt.short}
